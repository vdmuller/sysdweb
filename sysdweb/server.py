...
from sysdweb.systemd import Journal, systemdBus
...
def get_service_action(service, action):
    if service in config.sections():
        sdbus = (
            systemdBus(True)
            if config.get("DEFAULT", "scope", fallback="system") == "user"
            else systemdBus()
        )
        ...
        if action == "start":
            ...
            return (
                {action: "OK"}
                if sdbus.reload_or_restart_unit(unit)
                else {action: "Fail"}
            )
        elif action == "status":
...
def get_main():
    ...
    for service in config.sections():
        ...
        if service_status["status"] == "not-found":
            ...
            or service_status["status"] == "failed"
        ):
            cls = "danger"
    ...
    return template(
        "index",
        hostname=config.get("DEFAULT", "name", fallback=gethostname()),
        services=services,
    )
...
def get_service_journal_page(service):
    if service in config.sections():
        if get_service_action(service, "status")["status"] == "not-found":
            abort(
                400,
                "Sorry, but service '{}' unit not found in system.".format(
        ...
        return template(
            "journal",
            hostname=config.get("DEFAULT", "name", fallback=gethostname()),
            service=config.get(service, "title"),
...
def start(config_file, host, port):
    ...
    if port is None:
        port = config.get("DEFAULT", "port", fallback="10088")
